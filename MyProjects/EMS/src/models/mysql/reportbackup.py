...
async def current_power_dtl(cnx, log_dt_ref,company_id ,campus_id,bu_id ,plant_id ,plant_department_id ,equipment_group_id ,equipment_id,function_id ,meter_id ,group_for ,groupby ,period_id,from_date,to_date,shift_id,limit_report_for,limit_exception_for,limit_order_by ,limit_operation_value ,is_critical ,converter_id ,report_for,is_function , function_type ,reportfor,employee_id ,is_minmax,is_main_meter,is_demand,meter_type,is_plant_wise):    
...
'''
    ...
    if is_demand == 'yes':
        ...
        where +=f" and mm.max_demand <> 0 and mm.main_demand_meter = 'yes'"
    if report_for == 'detail' or report_for =='' :
    ...
    query = text(f'''
            SELECT                       
                ...
                ifnull(SUM(CASE WHEN mm.meter = 'equipment'AND mm.meter_type = 'primary' THEN  cp.equipment_kwh ELSE 0 END/CASE WHEN mm.meter = 'equipment'AND mm.meter_type = 'primary' THEN  cp.actual_ton ELSE 0 END),0) units_per_ton,
                ...
                '' as tooltip_kwh,
                '' as formula,
                ROUND(AVG(CASE WHEN mm.max_demand IS NOT NULL AND mm.max_demand <> 0 and mm.main_demand_meter = 'yes' THEN CASE WHEN mmf.kva = '*' THEN cp.kva * mmf.kva_value WHEN  mmf.kva = '/' THEN cp.kva / mmf.kva_value ELSE cp.kva END ELSE 0 END),prf.kva) AS demand,       
                IFNULL(AVG(CASE WHEN mm.max_demand IS NOT NULL AND mm.max_demand <> 0  and mm.main_demand_meter = 'yes' THEN CASE WHEN mmf.avg_powerfactor = '*' THEN cp.avg_powerfactor * mmf.avg_powerfactor_value WHEN  mmf.avg_powerfactor = '/' THEN cp.avg_powerfactor / mmf.avg_powerfactor_value ELSE cp.avg_powerfactor END ELSE 0 END),0) AS dm_powerfactor,             
                ROUND(AVG(CASE WHEN mm.max_demand IS NOT NULL AND mm.max_demand <> 0 and mm.main_demand_meter = 'yes' THEN CASE WHEN mmf.kva = '*' THEN cp.actual_demand * mmf.kva_value WHEN  mmf.kva = '/' THEN cp.actual_demand / mmf.kva_value ELSE cp.actual_demand END ELSE 0 END),prf.kva) AS actual_demand,            
		        IFNULL(MIN(CASE WHEN mm.max_demand IS NOT NULL AND mm.max_demand <> 0 and mm.main_demand_meter = 'yes' THEN cp.demand_dtm ELSE NULL END),'1900-01-01T00:00:00') AS d_date_time,
...
async def demand_report(period_id,campus_id,meter_id,from_date,to_date,shift_id,from_time,to_time,filter_type,parameter,duration,main_demand_meter,date_time,cnx):
    ...
    if period_id == "cur_shift":               
        ...
        if from_date != '' and to_date != '':
            if from_date.month == to_date.month and from_date.year == to_date.year:
                ...
                field_name = 'id,mill_date,mill_shift,kva,actual_demand,demand_dtm,avg_powerfactor,created_on'
    ...
    if main_demand_meter == 'yes':
        ...
        group_by = f" group by mp.campus_id,cp.created_on "
    else:
        group_by = f" group by mm.meter_id,cp.created_on"

    if date_time != '' and period_id == "peak_time":
        where +=f" and cp.created_on ='{date_time}'"
...
async def avg_demand_report(cnx,campus_id,period_id,from_date,to_date,meter_id,main_demand_meter):
    ...
    query = f'''
            select 
                ...
                mcs.campus_name,
                mm.max_demand,
                IFNULL(ROUND(AVG(CASE WHEN mmf.avg_powerfactor = '*' THEN cp.avg_powerfactor * mmf.avg_powerfactor_value WHEN  mmf.avg_powerfactor = '/' THEN cp.avg_powerfactor / mmf.avg_powerfactor_value ELSE cp.avg_powerfactor END),prf.avg_powerfactor),'') AS dm_avg_powerfactor,       
