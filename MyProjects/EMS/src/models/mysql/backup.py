...
async def current_power_dtl(cnx, log_dt_ref,company_id ,campus_id,bu_id ,plant_id ,plant_department_id ,equipment_group_id ,equipment_id,function_id ,meter_id ,group_for ,groupby ,period_id,from_date,to_date,shift_id,limit_report_for,limit_exception_for,limit_order_by ,limit_operation_value ,is_critical ,converter_id ,report_for,is_function , function_type ,reportfor,employee_id ,is_minmax,is_main_meter,is_demand,group,meter_type):    
    
    log_date_time = datetime.now()
    log_dt_start_model = log_date_time.strftime("%d/%m/%Y %H:%M:%S.%f")[:-3]
    createFolder("Log/API_dt/",f"current_power | Ref No : {log_dt_ref} | model function start dt : {log_dt_start_model}")
    ...
    if  company_id == '' or company_id == "0" or company_id == 'all':
        ...
        where_minmax += f" and  mm.company_id in ({company_id})" 
    ...
    if  bu_id == '' or bu_id == "0" or bu_id == 'all':
        ...
        where_minmax += f" and  mm.bu_id in ({bu_id})" 
    ...
    if plant_id == '' or plant_id == "0" or plant_id == 'all' or is_demand == 'yes':
        ...
        where_minmax += f" and  mm.plant_id in ({plant_id})"          
    ...
    if plant_department_id == '' or plant_department_id == "0" or plant_department_id == 'all':
        ...
        where_minmax += f" and mm.plant_department_id in ({plant_department_id})"
    ...
    if equipment_id == '' or equipment_id == 0 or equipment_id == 'all':
        ...
        where_minmax += f" and me.equipment_id in ({equipment_id})"
    ...
    if meter_id == '' or meter_id == 'all':
        ...
        where_minmax += f" and cp.meter_id in ({meter_id})"
    ...
    if reportfor == '12to12':
        ...
        if period_id == "cur_shift":               
            ...
            where += f''' and cp.mill_date = '{mill_date}' AND cp.mill_shift = '{mill_shift}' '''              
            where_minmax += f''' and cp.mill_date = '{mill_date}' AND cp.mill_shift = '{mill_shift}' '''              
            table_name = "ems_v1.current_power cp" 
            ...
            tablename=f"  {completed_db}power_{month_year} " 
            ...
            where += f''' and cp.mill_date = '{from_date}' AND cp.mill_shift = '{shift_id}' '''   
            where_minmax += f''' and cp.mill_date = '{from_date}' AND cp.mill_shift = '{shift_id}' '''   
            where_p += f''' and cpd.mill_date = '{from_date}' AND cpd.mill_shift = '{shift_id}' '''   
            result_p = await check_polling_data_tble(cnx,month_year)
            ...
            tablename=f"  {completed_db}power_{month_year} "
            where += f''' and cp.mill_date = '{from_date}' '''
            where_minmax += f''' and cp.mill_date = '{from_date}' '''
            where_p += f''' and cpd.mill_date = '{from_date}' '''
            
        elif period_id  == "#this_week":
            where += f''' and cp.mill_date  >= '{from_date}' and cp.mill_date <= '{to_date}' '''
            where_minmax += f''' and cp.mill_date  >= '{from_date}' and cp.mill_date <= '{to_date}' '''
    ...
    if is_minmax == 'yes':
        query1 += f'''
                (SELECT mill_date FROM {tablename} as cp where 1=1 {where_minmax} ORDER BY cp.kwh ASC LIMIT 1) AS min_date,
                (SELECT mill_date FROM {tablename} as cp where 1=1 {where_minmax} ORDER BY cp.kwh DESC LIMIT 1) AS max_date,
                (SELECT mill_shift FROM {tablename} as cp where 1=1 {where_minmax} ORDER BY cp.kwh ASC LIMIT 1) AS min_shift,
                (SELECT mill_shift FROM {tablename} as cp where 1=1 {where_minmax} ORDER BY cp.kwh DESC LIMIT 1) AS max_shift,
    ...
    query = text(f'''
            SELECT                       
                ...
                ROUND(AVG(CASE WHEN mm.meter_type = 'Primary' THEN CASE WHEN mmf.avg_powerfactor = '*' THEN cp.avg_powerfactor * mmf.avg_powerfactor_value WHEN  mmf.avg_powerfactor = '/' THEN cp.avg_powerfactor / mmf.avg_powerfactor_value ELSE cp.avg_powerfactor END ELSE 0 END ),prf.avg_powerfactor) AS pm_avg_powerfactor,
                ...
                ifnull(SUM(CASE WHEN mm.meter = 'equipment'AND mm.meter_type = 'primary' THEN  cp.equipment_kwh ELSE 0 END)/SUM(CASE WHEN mm.meter = 'equipment'AND mm.meter_type = 'primary' THEN  cp.actual_ton ELSE 0 END),0) units_per_ton,
                ...
                '' as tooltip_kwh,
                '' as formula,
                ROUND(AVG(CASE WHEN mm.max_demand IS NOT NULL AND mm.max_demand <> 0 THEN CASE WHEN mmf.kva = '*' THEN cp.kva * mmf.kva_value WHEN  mmf.kva = '/' THEN cp.kva / mmf.kva_value ELSE cp.kva END ELSE 0 END),prf.kva) AS demand,       
                AVG(CASE WHEN mm.max_demand IS NOT NULL AND mm.max_demand <> 0 THEN CASE WHEN mmf.avg_powerfactor = '*' THEN cp.avg_powerfactor * mmf.avg_powerfactor_value WHEN  mmf.avg_powerfactor = '/' THEN cp.avg_powerfactor / mmf.avg_powerfactor_value ELSE cp.avg_powerfactor END ELSE 0 END) AS dm_powerfactor,       
                ROUND(AVG(CASE WHEN mm.max_demand IS NOT NULL AND mm.max_demand <> 0 THEN CASE WHEN mmf.kva = '*' THEN cp.actual_demand * mmf.kva_value WHEN  mmf.kva = '/' THEN cp.actual_demand / mmf.kva_value ELSE cp.actual_demand END ELSE 0 END),prf.kva) AS actual_demand,       
		        IFNULL(MIN(CASE WHEN mm.max_demand IS NOT NULL AND mm.max_demand <> 0 THEN cp.demand_dtm ELSE NULL END),'1900-01-01T00:00:00') AS d_date_time,
                SUM(mm.max_demand) AS max_demand,  
                mm.meter,  
                mdl.model_name,
                mk.model_make_name,
                SUM(CASE WHEN mm.meter = 'equipment'AND mm.meter_type = 'primary' THEN cp.actual_ton else 0 end ) actual_ton,          
                {poll_duration}
                {group_id}
                {group_code}
                {group_name}                       
            ...
            ''')
    log_date_time = datetime.now()
    log_dt_start_model_sql = log_date_time.strftime("%d/%m/%Y %H:%M:%S.%f")[:-3]
    createFolder("Log/API_dt/",f"current_power | Ref No : {log_dt_ref} | model function query start dt : {log_dt_start_model_sql}")

    # if groupby == "meters" or groupby == "bu":
    createFolder("Current_power_log/","current_power api query "+str(query))
    datas = await cnx.execute(query)
    datas = datas.mappings().all()

    log_date_time = datetime.now()
    log_dt_end_model = log_date_time.strftime("%d/%m/%Y %H:%M:%S.%f")[:-3]
    createFolder("Log/API_dt/",f"current_power | Ref No : {log_dt_ref} | model function end dt : {log_dt_end_model}")
...
async def communication_status_query(cnx,plant_department_id,plant_id):
    ...
    query = text(f'''
                ...
                from
                    ...
                    ems_v1.master_converter_detail c
                where
                    mm.converter_id = c.converter_id 
                    {where}
...
async def month_report(cnx,company_id,bu_id,plant_id,plant_department_id,equipment_group_id,employee_id,meter_id,month_year,report_for,report_type,request):
        ...
        query = text(f'''
            ...
            FROM
                ...
                inner JOIN ems_v1.master_parameter_roundoff prf ON prf.plant_id = md.plant_id                   
            WHERE
                1=1 {where} and DATE_FORMAT(cp.mill_date ,'%m-%Y') = '{month_year}' 
            GROUP BY
                mm.meter_code,
                mm.meter_name,
                DAY(cp.mill_date)
        ...
        if rslt !='':
            ...
            # meter = {"result" : data["meter_name"],
            #            "result1" : data[]}
            createFolder("Log/","result"+str(result))
        return result
...
async def year_wise_report_print(cnx,meter_id,year,report_for,employee_id,request):
...
async def hour_wise_analysis_report(cnx,company_id,bu_id,plant_id,meter_id,from_time,to_time):
        ...
        query=text(f'''
            SELECT
                mm.meter_id,
                min(mm.meter_code) as meter_code,
                min(mm.meter_name) as meter_name,
            ...
            FROM
                ...
                INNER JOIN ems_v1.master_meter mm ON mm.meter_id = cp.meter_id
                INNER JOIN ems_v1.master_meter_factor mmf ON mmf.meter_id = mm.meter_id and mmf.plant_id = mm.plant_id 
                INNER JOIN ems_v1.master_parameter_roundoff prf ON mm.plant_id = prf.plant_id
            WHERE mm.meter_id in ({meter_id}) {where}
            group by mm.meter_id
          ''') 
        data = await cnx.execute(query)
        data = data.fetchall() 
...
async def gateway_log(cnx,mac,from_date,from_time,to_time):
    ...
    for row in result:
        ...
        for data in modbus_data:
            ...
            packet = {'slave_id': slave_id, 'is_error': is_error,'data':f'{data}','mac':mac,'date_time':date_time}
...
async def route_card_kwh(cnx,equipment_id,from_date,to_date,groupby):
    ...
    if groupby == "equipment":
        group_by = f"group by mm.equipment_id "
    ...
    if from_date.month != to_date.month :
        ...
        table = f"( select {fields} from current_power_analysis {subquery_union}) "
...
async def demand_report(period_id,campus_id,from_date,to_date,shift_id,from_time,to_time,filter_type,parameter,duration,cnx):
    ...
    if parameter == 'pf' and filter_type != 'all':
        ...
        query2 = '''
                    ...
                    FROM RankedData
                    WHERE row_num = 1'''
        partition = ',ROW_NUMBER() OVER (PARTITION BY cp.avg_powerfactor ORDER BY cp.created_on) AS row_num'
    ...
    elif filter_type == 'all' and duration != '':
    ...
    query = F'''
            ...
            select
                ...
                mp.campus_id,
                mcs.campus_name,
                MAX(mm.max_demand) as max_demand,
                cp.mill_date,
                cp.mill_shift,
                ROUND(SUM(CASE WHEN mm.max_demand IS NOT NULL AND mm.max_demand <> 0 THEN CASE WHEN mmf.kva = '*' THEN cp.kva * mmf.kva_value WHEN  mmf.kva = '/' THEN cp.kva / mmf.kva_value ELSE cp.kva END ELSE 0 END),prf.kva) AS demand,       
                ROUND(SUM(CASE WHEN mm.max_demand IS NOT NULL AND mm.max_demand <> 0 THEN CASE WHEN mmf.kva = '*' THEN cp.actual_demand * mmf.kva_value WHEN  mmf.kva = '/' THEN cp.actual_demand / mmf.kva_value ELSE cp.actual_demand END ELSE 0 END),prf.kva) AS actual_demand,
                MAX(CASE WHEN mm.max_demand IS NOT NULL AND mm.max_demand <> 0 THEN cp.demand_dtm ELSE NULL END) AS d_date_time,
                SUM(CASE WHEN mm.max_demand IS NOT NULL AND mm.max_demand <> 0 THEN CASE WHEN mmf.avg_powerfactor = '*' THEN cp.avg_powerfactor * mmf.avg_powerfactor_value WHEN  mmf.avg_powerfactor = '/' THEN cp.avg_powerfactor / mmf.avg_powerfactor_value ELSE cp.avg_powerfactor END ELSE 0 END) AS dm_powerfactor, 
                cp.created_on as date_time
                {partition}
                ...
                from 
                    ...
                    inner JOIN ems_v1.master_parameter_roundoff prf ON prf.plant_id = mp.plant_id 
                    {current_shift}
                where mm.status = 'active' AND mm.max_demand <>0 {where}
                group by mp.campus_id,cp.created_on
                {query2}
                '''
    datas = await cnx.execute(query)
    datas = datas.mappings().all()
...
async def availability_report(cnx,campus_id,from_date,to_date,from_year,to_year):
    ...
    if campus_id != '':
        where += f" and msed.campus_id ='{campus_id}'"
    ...
    query = f'''
            select
                SUM(msed.consumption) AS consumption,
                mse.*,
                msed.mill_date
            FROM 
    ...
    query2 = f'''
            ...
            where mbe.status = 'active' and mbe.financial_year >= '{from_year}' and mbe.financial_year<={to_year} {where1}
