    ...
    if reportfor == '12to12':
        if period_id == "sel_date":            
            ...
            if from_date.month == to_date.month:
                ...
                month_year_range = [
                    (from_date + timedelta(months=i)).strftime("%m%Y")
        ...
        if period_id == '#previous_week' or period_id == "#this_week" or period_id == "#this_month" or period_id == '#previous_month' or period_id=="#previous_year" or period_id=="#this_year" or period_id=="from_to" or  period_id=="#from_to" or period_id == "#sel_year":
            if from_date != '' and to_date != '':
                if from_date.month == to_date.month and from_date.year == to_date.year:
                    ...
                    field_name = 'power_id,company_id,bu_id,plant_id,plant_department_id,equipment_group_id,meter_id,design_id,beam_id,date_time,date_time1,mill_date,mill_shift,vln_avg,r_volt,y_volt,b_volt,vll_avg,ry_volt,yb_volt,br_volt,t_current,r_current,y_current,b_current,t_watts,r_watts,y_watts,b_watts,t_var,r_var,y_var,b_var,t_voltampere,r_voltampere,y_voltampere,b_voltampere,avg_powerfactor,r_powerfactor,y_powerfactor,b_powerfactor,powerfactor,kWh,kvah,kw,kvar,power_factor,kva,frequency,machine_status,status,created_on,created_by,modified_on,modified_by,machine_kWh,master_kwh,reverse_machine_kWh,reverse_master_kwh,reverse_kwh,meter_status_code,equipment_kwh,actual_demand,demand_dtm,actual_ton,runhour,r_volt_thd,b_volt_thd,y_volt_thd,avg_volt_thd,r_current_thd,y_current_thd,b_current_thd,avg_current_thd,ry_volt_thd,yb_volt_thd,br_volt_thd,vll_avg_thd,vln_avg_thd'
                    field_name1 = 'meter_id,mill_date,mill_shift,kWh,meter_status,poll_duration,poll_consumption,equipment_consumption'
                    month_year_range = [
                    (from_date + relativedelta(months=i)).strftime("%m%Y")
                    for i in range((to_date.year - from_date.year) * 12 + to_date.month - from_date.month + 1)
                ]
...
'''
        ...
        if period_id == "cur_shift":
            ...
            if len(result_p) > 0: 
                poll_duration = ''' TIME_FORMAT(SEC_TO_TIME(SUM(cpd.off_time)),'%H:%i:%s') AS off_time,
                                    ...
                                    ROUND(ifnull(sum(cpd.off_kwh),0)) as off_kwh,
                                    ROUND(ifnull(sum(cpd.on_load_kwh),0)) as on_load_kwh,
                                    ROUND(ifnull(sum(cpd.idle_kwh),0)) as idle_kwh,
                '''
    ...
    query = text(f'''
            SELECT                       
                ...
                ROUND(SUM(case when mmf.machine_kWh = '*' then cp.machine_kWh * mmf.machine_kWh_value when  mmf.machine_kWh = '/' then cp.machine_kWh / mmf.machine_kWh_value else cp.machine_kWh end )) AS machine_kWh,
                ROUND(SUM(case when mmf.machine_kWh = '*' then cp.master_kwh * mmf.machine_kWh_value when  mmf.machine_kWh = '/' then cp.master_kwh / mmf.machine_kWh_value else cp.master_kwh end )) AS master_kwh,
                ROUND(SUM(case when mmf.kWh = '*' then cp.kWh * mmf.kWh_value when  mmf.kWh = '/' then cp.kWh / mmf.kWh_value else cp.kWh end )) AS kWh,
                ...
                ROUND(SUM(case when mmf.kWh = '*' then cp.kWh * mmf.kWh_value when  mmf.kWh = '/' then cp.kWh / mmf.kWh_value else cp.kWh end )) AS total_kWh,
                ROUND(MIN(case when mmf.kWh = '*' then cp.kWh * mmf.kWh_value when  mmf.kWh = '/' then cp.kWh / mmf.kWh_value else cp.kWh end )) AS kwh_min,
                ROUND(MAX(case when mmf.kWh = '*' then cp.kWh * mmf.kWh_value when  mmf.kWh = '/' then cp.kWh / mmf.kWh_value else cp.kWh end )) AS kwh_max,
                ROUND(AVG(case when mmf.kWh = '*' then cp.kWh * mmf.kWh_value when  mmf.kWh = '/' then cp.kWh / mmf.kWh_value else cp.kWh end )) AS avg_kWh,
                {query1}
                
                ROUND(SUM(case when mmf.machine_kWh = '*' then cp.reverse_machine_kWh * mmf.machine_kWh_value when  mmf.machine_kWh = '/' then cp.reverse_machine_kWh / mmf.machine_kWh_value else cp.reverse_machine_kWh end )) AS reverse_machine_kWh,
                ROUND(SUM(case when mmf.machine_kWh = '*' then cp.reverse_master_kwh * mmf.machine_kWh_value when  mmf.machine_kWh = '/' then cp.reverse_master_kwh / mmf.machine_kWh_value else cp.reverse_master_kwh end )) AS reverse_master_kwh,
                ROUND(SUM(case when mmf.kWh = '*' then cp.reverse_kwh * mmf.kWh_value when  mmf.kWh = '/' then cp.reverse_kwh / mmf.kWh_value else cp.reverse_kwh end )) AS reverse_kwh,
                ...
                ROUND(SUM(CASE WHEN cp.mill_shift = 1 THEN case when mmf.kWh = '*' then cp.kwh * mmf.kwh_value when  mmf.kwh = '/' then cp.kwh / mmf.kwh_value else cp.kwh end ELSE 0 END)) AS kwh_1,
                ROUND(SUM(CASE WHEN cp.mill_shift = 2 THEN case when mmf.kWh = '*' then cp.kwh * mmf.kwh_value when  mmf.kwh = '/' then cp.kwh / mmf.kwh_value else cp.kwh end ELSE 0 END)) AS kwh_2,
                ROUND(SUM(CASE WHEN cp.mill_shift = 3 THEN case when mmf.kWh = '*' then cp.kwh * mmf.kwh_value when  mmf.kwh = '/' then cp.kwh / mmf.kwh_value else cp.kwh end ELSE 0 END)) AS kwh_3,
                ROUND(min(CASE WHEN cp.mill_shift = 1 THEN case when mmf.machine_kWh = '*' then cp.master_kwh * mmf.machine_kWh_value when  mmf.machine_kWh = '/' then cp.master_kwh / mmf.machine_kWh_value else cp.master_kwh end  END)) AS start_kwh_1,
                ROUND(max(CASE WHEN cp.mill_shift = 2 THEN case when mmf.machine_kWh = '*' then cp.master_kwh * mmf.machine_kWh_value when  mmf.machine_kWh = '/' then cp.master_kwh / mmf.machine_kWh_value else cp.master_kwh end  END)) AS start_kwh_2,
                ROUND(max(CASE WHEN cp.mill_shift = 3 THEN case when mmf.machine_kWh = '*' then cp.master_kwh * mmf.machine_kWh_value when  mmf.machine_kWh = '/' then cp.master_kwh / mmf.machine_kWh_value else cp.master_kwh end  END)) AS start_kwh_3,     
                ROUND(min(CASE WHEN cp.mill_shift = 1 THEN case when mmf.machine_kWh = '*' then cp.machine_kWh * mmf.machine_kWh_value when  mmf.machine_kWh = '/' then cp.machine_kWh / mmf.machine_kWh_value else cp.machine_kWh end  END)) AS end_kwh_1,
                ROUND(max(CASE WHEN cp.mill_shift = 2 THEN case when mmf.machine_kWh = '*' then cp.machine_kWh * mmf.machine_kWh_value when  mmf.machine_kWh = '/' then cp.machine_kWh / mmf.machine_kWh_value else cp.machine_kWh end  END)) AS end_kwh_2,
                ROUND(max(CASE WHEN cp.mill_shift = 3 THEN case when mmf.machine_kWh = '*' then cp.machine_kWh * mmf.machine_kWh_value when  mmf.machine_kWh = '/' then cp.machine_kWh / mmf.machine_kWh_value else cp.machine_kWh end  END)) AS end_kwh_3,
                ...
                ROUND(SUM(CASE WHEN mm.meter = 'equipment' and mm.is_poll_meter = 'yes' THEN  cp.equipment_kwh  ELSE 0 END)) AS equipment_kwh,
                ROUND(SUM(CASE WHEN mm.meter = 'equipment' and mm.is_poll_meter = 'yes' THEN  cp.equipment_kwh  ELSE 0 END),0) AS units,
                ROUND(SUM(CASE WHEN mm.meter = 'common' THEN cp.equipment_kwh ELSE 0 END)) AS common_kwh,
                ROUND(SUM(CASE WHEN mm.meter = 'equipment'and mm.meter_type = 'primary' THEN  cp.equipment_kwh ELSE 0 END)) AS pm_equipment_kwh,
                ...
                ROUND(SUM(CASE WHEN mm.meter = 'common' and mm.meter_type = 'primary' THEN  cp.equipment_kwh  ELSE 0 END)) AS pm_common_kwh,
...
async def availability_report(cnx,campus_id,from_date,to_date,from_year,to_year,report_type):
    ...
    query = f'''
            select
                ...
                ifnull(SUM(msed.consumption_total),0) AS consumption_total,
                mse.*,
                ifnull(mbr.budget,0) budget,
                ifnull(mbr.actual,0) actual,
                ifnull(mbr.actual_total,0) actual_total,
...
async def transformerlossreport(cnx,campus_id,from_date,to_date):
    ...
    sql = f'''
            select 
                mm.meter_id,
                mm.meter_name,
                CONCAT(mm.meter_code , '-' , mm.meter_name, ' - ', mp.plant_name) AS meter_code,
                c.campus_name,
                mp.campus_id,
                mp.plant_name,
                ROUND(SUM(case when mmf.kWh = '*' then cp.kWh * mmf.kWh_value when  mmf.kWh = '/' then cp.kWh / mmf.kWh_value else cp.kWh end ),prf.kWh) AS kWh,
            ...
            from 
                ...
                inner JOIN ems_v1.master_meter_factor mmf ON  mmf.plant_id = mp.plant_id AND mmf.meter_id = mm.meter_id
                inner JOIN ems_v1.master_parameter_roundoff prf ON prf.plant_id = mp.plant_id 
            where mm.status = 'active' and cp.mill_date >='{from_date}' and cp.mill_date <='{to_date}'  and mm.main_transformer_meter = 'yes' {where}
            group by mm.meter_id, cp.mill_date
            order by mm.plant_id,mm.meter_id,cp.mill_date
                '''
    ...
    if rslt !='':
        ...
        for row in rslt:
            ...
            # Initialize day and calculated_kwh to 0 if not already present
                for key, cal_kwh_key in zip(output_keys, cal_kwh_keys):
                    output[meter_code][key] = 0
                    output[meter_code][cal_kwh_key] = 0
                    output[meter_code]["total_kwh"] = 0
                    output[meter_code]["total_cal"] = 0

            output[meter_code][f'd{day}'] = kwh
            output[meter_code]["total_kwh"] += kwh
            output[meter_code][f'cal_kwh_d{day}'] = calculated_kwh
            output[meter_code]["total_cal"] += calculated_kwh
...
async def submeterlossreport(cnx,campus_id,main_transformer_meter_id,from_date,to_date):
    where = ''
    if campus_id != '':
        where += f" and mp.campus_id ='{campus_id}' "
        
    if main_transformer_meter_id != '':
        where += f" and tm.main_transformer_meter_id in ('{main_transformer_meter_id}') "
    ...
    sql = f'''
            ...
            from 
                {table_name}
                inner join master_transformer_meter tm on tm.sub_meter_id = cp.meter_id
                inner join master_meter mm on mm.meter_id = tm.sub_meter_id
                inner join master_plant mp on mp.plant_id = mm.plant_id
                inner join master_campus c on c.campus_id = mp.campus_id
                inner JOIN ems_v1.master_meter_factor mmf ON  mmf.plant_id = mp.plant_id AND mmf.meter_id = mm.meter_id
                inner JOIN ems_v1.master_parameter_roundoff prf ON prf.plant_id = mp.plant_id 
            where mm.status = 'active' and cp.mill_date >='{from_date}' and cp.mill_date <='{to_date}'  {where}
            group by mm.meter_id, cp.mill_date
            order by mm.plant_id,tm.main_transformer_meter_id,mm.meter_id,cp.mill_date
                '''
    rslt = await cnx.execute(sql)
    rslt = rslt.fetchall()
    if rslt !='':
        ...
        for row in rslt:
            ...
            campus_name = row.campus_name
            plant_name = row.plant_name
            main_transformer_meter_id = row.main_transformer_meter_id
            day = row.day
            ...
            if meter_code not in output:
                output[meter_code] = {
                    ...
                    "campus_name": campus_name,
                    "plant_name": plant_name,   
                    "main_transformer_meter_id": main_transformer_meter_id    
                }
