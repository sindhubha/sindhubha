...
if os.path.exists(file_path):
    ...
    if content == 'MySQL':
        ...
        from src.models.mysql.report_model import month_report,daily_report,year_wise_report_print,year_report_print,holiday_report,alarmreport,hour_wise_analysis_report,get_hour_wise_report_model,import_export_dtl,current_power_dtl,function_dashboard,function_dashboard2,communication_status_query,get_equipment_cal_dtl,gateway_log,route_card_kwh,demand_report,availability_report,tnebreportdetail,avg_demand_report,minmax_kwh_dtl,manualentryhistory,transformerlossreport
...
def availabilityreport_excel(data,from_date,to_date,report_type,employee_name,month):
    wb = Workbook()
    ws = wb.active
    border_style = Border(left=Side(style='thin'), 
                     right=Side(style='thin'), 
    ...
    if report_type == 'with_rate':
        ...
        energy_statement = f"Energy Statement With Tariff {month}"
    else:
    ...
    ws.column_dimensions[cell[0]].width = len(energy_statement) + 2  # Adjust column width
    ...
    alignment = Alignment(horizontal="center", vertical="center")
    ws[cell].alignment = alignment
    ws.column_dimensions[cell[0]].width = 1   # Adjust column width
    font = Font(name='Calibri', size=12, color='FFFFFF', bold=True)
    ws[cell].font = font
    ws[cell].border = border_style 
    fill = PatternFill(start_color='1e98a3', end_color='1e98a3', fill_type='solid')
    ...
    alignment = Alignment(horizontal="center", vertical="center")
    ws[cell].alignment = alignment
    ws.column_dimensions[cell[0]].width = len("%") + 15  # Adjust column width
    font = Font(name='Calibri', size=12, color='FFFFFF', bold=True)
    ws[cell].font = font
    ws[cell].border = border_style 
    fill = PatternFill(start_color='1e98a3', end_color='1e98a3', fill_type='solid')
...
def availabilityreport_pdf(data, from_date, to_date,report_type,employee_name,month):
    ...
    if report_type == 'with_rate':
        table_data = [
            ...
            ["External", "", "", "Budget", "Actual", "Budget", "Actual", "Rate/Unit", "Total Amount",'Mix%','Mix Units','Mix Amount','Total Amount']
        ]
        ...
        for row in data["data"]:
            ...
            table_data.append([energy_source_name, consumption, ""])
        ...
        for i, row in enumerate(table_data):
            if row[0] == 'TNEB':
                table_data[i][1] = tneb_consumption
                table_data[i][2] = round((tneb_consumption/EI_total)* 100,2)
...
async def get_availability_report(request: Request,
    ...
    try:
        ...
        if report_for == "pdf":
            availabilityreport_pdf(data, from_date, to_date,report_type,employee_name,month)
            file_name = f'availability_report.pdf'
        else:
            availabilityreport_excel(data,start_date,end_date,report_type,employee_name,month)
            file_name = f'availability_report.xlsx'
        results = f"http://{request.headers['host']}/attachments/{file_name}"
...
async def transformerlossreportexcel(data,from_date,to_date,employee_name):
    month_year=f"""{mill_month[from_date.month]}-{str(from_date.year)}""" 
    ...
    if data == []:
        ...
        Heading = f"Transformer wise Distribution Loss Report {from_date} TO {to_date}"
        ws[cell] = Heading
        alignment = Alignment(horizontal="center", vertical="center")
        ws[cell].alignment = alignment 
        ws[cell].border = border_style
        ...
        ws.column_dimensions[cell[0]].width = 5  # Adjust column width
        ...
        fill = PatternFill(start_color='1e98a3', end_color='1e98a3', fill_type='solid')
        ws[cell].fill = fill
        ws.column_dimensions[cell[0]].width = 25  # Adjust column width
        ...
        unique_meter_codes = sorted(set(item["meter_code"] for item in data))

        i=0
        cols = 0
        for meter_code in sorted(unique_meter_codes):
            ...
            header_cell.font = Font(bold=True, name='Calibri', size=12,color = "FFFFFF")
            header_cell.alignment = Alignment(horizontal="center", vertical="center")
        ...
        set_row = i * 3
        cols = set_row+4
        print(header_cell.alignment)
        headers = ['', "slno","date"]
        for meter_code in unique_meter_codes:
            headers.extend(["kwh", "dist_loss", "%"])
        headers.extend(["kwh", "dist_loss", "%"])
        ws.append(headers)
        for col in range(1, len(headers) + 1):  # Assuming headers is a list of column indices
        ...
        meter_totals_kwh = {meter_code: 0 for meter_code in unique_meter_codes}
        meter_totals_dist_loss = {meter_code: 0 for meter_code in unique_meter_codes}
        overall_kwh = 0
        overall_cal_kwh = 0
        for row in range(1, days_in_month + 1):
            ...
            col_kwh_value = 0
            col_cal_kwh_value = 0
            # Iterate over data to fill kWh values
            for meter_code in unique_meter_codes:
                meter_total_kwh = 0  # Total kWh for this meter code
                for item in data:
                    campus_name = item["campus_name"]
                    if item["meter_code"] == meter_code:
                        kwh_key = f"d{row}"
                        ...
                        if kwh_key in item and cal_kwh_key in item:
                            ...
                            col_kwh_value +=kwh_value
                            col_cal_kwh_value +=cal_kwh_value
                            # Update meter totals for this row
            ...
            row_data.append(round(col_kwh_value))
            row_data.append(round(col_cal_kwh_value))
            if col_kwh_value!=0:
                cal_total_percentage_value = round(col_cal_kwh_value / col_kwh_value * 100,2)
            else:
                cal_total_percentage_value = 0
            row_data.append(cal_total_percentage_value)
            overall_kwh +=col_kwh_value
            overall_cal_kwh +=col_cal_kwh_value
            ws.append(row_data)
        ...
        totals_row = ['', 'Total','']
        for meter_code in unique_meter_codes:
            totals_row.append(round(meter_totals_kwh[meter_code]))
            totals_row.append(round(meter_totals_dist_loss[meter_code]))
            kwh_total_value = meter_totals_kwh[meter_code]
            cal_kwh_total_value = meter_totals_dist_loss[meter_code]
            if kwh_total_value!=0:
                total_percentage_value = round(cal_kwh_total_value / kwh_total_value * 100,2)
            else:
                total_percentage_value = 0
            totals_row.append(total_percentage_value)
        totals_row.append(round(overall_kwh))
        totals_row.append(round(overall_cal_kwh))
        if overall_kwh!=0:
                overall_percentage_value = round(overall_cal_kwh / overall_kwh * 100,2)
        else:
            overall_percentage_value = 0
        totals_row.append(overall_percentage_value)
    ...
    file_name = f'DailyDistribution_Loss.xlsx'
    file_path = os.path.join(static_dir, file_name)
    wb.save(file_path)
...
async def transformer_wise_loss_report(request:Request,
    ...
    try:
        ...
        await transformerlossreportexcel(data,from_date,to_date,employee_name)
        file_name = f"DailyDistribution_Loss.xlsx"
        results = f"http://{request.headers['host']}/attachments/{file_name}"
        response = {
                    "iserror": False,
...
# @router.post("/current_flow/", tags=["Dashboard"])
