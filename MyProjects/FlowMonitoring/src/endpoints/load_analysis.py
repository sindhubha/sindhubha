...
if os.path.exists(file_path):
    ...
    if content == 'MySQL':
        ...
        from src.models.mysql.load_analysis_model import load_analysis_mdl,polling_analysis,fuel_analysis_mdl
...
@router.post("/fuel_analysis/", tags=["Analysis"])
async def fuel_analysis(company_id : int = Form(""),
                        bu_id: int = Form(''),
                        plant_id: int = Form(''),
                        period_id: str = Form(''),
                        group_by: str = Form(''),
    ...
    try: 
        ...
        if period_id == 'cur_shift' or period_id == '#cur_shift':
            ...
            res = await f"check_{source}analysis_table"(cnx,month_year)
            if len(res) == 0:
                return _getErrorResponseJson("Analysis Table Not Available...") 
        ...
        if period_id == '#previous_week' or period_id == "#this_week" or period_id == "#this_month" or period_id == '#previous_month' or period_id=="#previous_year" or period_id=="#this_year" or period_id=="from_to" or period_id == "#sel_year" or period_id=="#from_to" :
                ...
                if from_date != '' and to_date != '':
                    ...
                    for month_year in month_year_range:
                        res = await f"check_{source}_analysis_table"(cnx,month_year)
                    ...
                    if len(union_queries) == 0:
                        return _getErrorResponseJson(f"{source} Analysis Table Not Available...")     
            
        if group_by == 'equipment' and equipment_id == "" :
            return _getErrorResponseJson("Equipment Id Is Required...")  
        
        data = await fuel_analysis_mdl(cnx,company_id,bu_id,plant_id,period_id,group_by,equipment_id,meter_id,from_date,to_date,shift_id,from_time,to_time,duration,source)
