...
async def get_power_report_fields(plant_id : int = Form(''),
    ...
    try: 
        ...
        if report_id == 0:
            result = await power_report_fields(cnx,plant_id,fields_for,is_all_field)
        else:
            result = await power_reportfield(cnx,plant_id,report_id,fields_for,is_all_field)
...
async def update_power_report_fields(plant_id : int = Form(''),
    ...
    try:
        ...
        await update_power_report(cnx,plant_id,report_id,obj,fields_for)
