            ...
            if int(no_of_shifts) == 1:
                ...
                if shift1_start[:10] != shift1_end[:10]:
                    ...
                    if curtime1[11:] >= shift1_start[11:] and curtime1[11:] < '23:59:59':
                        ...
                        if '00:00:00' < shift1_end[11:]:
                            act_shift = 1
                            act_date = date.today() - timedelta(days=1)
                            shift_start = shift1_start[11:]
                            return act_date , act_shift , shift_start , company_date , company_shift
...
def shift_end(company_id,bu_id,plant_id,db,cursor):
    ...
    try:
        ...
        global act_date, act_shift
        createFolder(f"{Logfile_name}{plant_id}/", "Shift End Called !")

        cursor.execute(f"SELECT mill_date,mill_shift from ems_v1.master_shifts_exe where company_id = '{company_id}' and bu_id = '{bu_id}' and plant_id = '{plant_id}' and created_for = 'datalogger'")
        rows=cursor.fetchall()
        for row in rows:
        ...
        KEY `machine` (`meter_id`,`company_id`,`bu_id`,`plant_id`,`plant_department_id`,`equipment_group_id`,`date_time`,`mill_date`,`mill_shift`)
        )
        '''
        cursor.execute(sql)
        db.commit()
        ...
        `meter_status` int(11) NOT NULL DEFAULT 0,
        PRIMARY KEY (`id`),
        KEY `machine` (`meter_id`,`id`,`company_id`,`bu_id`,`plant_id`,`plant_department_id`,`equipment_group_id`,`date_time`,`mill_date`,`mill_shift`,`created_on`),
        KEY `idx_power_analysis_created_on_meter_id` (`created_on`,`meter_id`),
        KEY `idx_fk_cp_meter_id` (`meter_id`),
        KEY `idx_fk_cp_mill_date_id` (`mill_date`),
        KEY `idx_fk_cp_mill_shift` (`mill_shift`)
        ) 
        """
        cursor.execute(sql)
        db.commit()
        ...
        try:
            sql = f'''
                    ...
                    powerfactor = 0 , kwh = 0 , kvah = 0 , kw = 0 , kvar = 0 , power_factor = 0 , kva = 0 , frequency = 0 ,r_volt_thd = 0,y_volt_thd = 0,b_volt_thd = 0,avg_volt_thd = 0,r_current_thd = 0,y_current_thd = 0,b_current_thd = 0,avg_current_thd = 0
                    where company_id = '{company_id}' and bu_id = '{bu_id}' and plant_id = '{plant_id}' 
                    '''
        ...
        try:
            ...
            sql = f'''
                INSERT INTO gateway_completed.log_{act_month_year}(imei_no,mac,signal_strength,received_packet,date_time,msg_type,production_quantity,created_on,STATUS)
            ...
            query = f'''
                delete from gateway.log WHERE mac IN (SELECT mac FROM ems_v1.master_meter WHERE  plant_id = '{plant_id}') AND STATUS = 'yes' 
                '''
...
while True:
    ...
    try:
        ...
        # db = pymysql.connect(host="localhost", user="AIC_PY_SE",passwd="88f63b33874910309091e5aebcd06ead", db="ems_v1" , port= 3306)
