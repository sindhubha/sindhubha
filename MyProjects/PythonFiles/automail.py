...
def auto_mail(db,cursor):
    try:
        ...
        formatted_date = current_time.strftime("%d %H:%M")
        ...
        if len(automail_list)>0:
            for rows in automail_list:
                ...
                if report == 'Energy Statement':
                    ...
                    send_date = new_send_date.strftime("%d %H:%M")
                    print(send_date)
...
if process_count > 2:
    ...
    while True:
        ...
        try:
            ...
            db = pymysql.connect(host="localhost", user="AIC_AUTOMAIL",passwd="b985f17b32c6dec5fec4cb15c7417a46", db="ems_v1" , port= 3308)
